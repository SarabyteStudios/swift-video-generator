...
public class VideoGenerator: NSObject {
        ...
        if let documentsPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first {
            ...
            if let videoWriter = VideoGenerator.current.videoWriter {
                ...
                let videoSettings: [String : AnyObject] = [
                    ...
                    AVVideoWidthKey  : 368 as AnyObject,
                    AVVideoHeightKey : 216 as AnyObject,
                    ]
                
                print("Video size: ", outputSize)
    ...
    open func mergeVideoWithAudio(videoUrl: URL, audioUrl: URL, success: @escaping ((URL) -> Void), failure: @escaping ((Error) -> Void)) {
        ...
        mutableVideoComposition.renderSize = CGSize(width: 368, height: 216)
    ...
    fileprivate func setup(withImages _images: [UIImage], andAudios _audios: [URL], andType _type: VideoGeneratorType) {
        ...
        if self.type == .single {
            ...
            for _image in _images {
                autoreleasepool {
                    if let imageData = _image.scaleImageToSize(newSize: CGSize(width: 368, height: 216))?.pngData() {
        ...
        images = images.compactMap({ $0.scaleImageToSize(newSize: CGSize(width: 368, height: 216)) })
    ...
    private func fillPixelBufferFromImage(_ image: UIImage, pixelBuffer: CVPixelBuffer) {
        ...
        if let context = CGContext(data: pixelData, width: Int(368), height: Int(216), bitsPerComponent: 8, bytesPerRow: CVPixelBufferGetBytesPerRow(pixelBuffer), space: rgbColorSpace, bitmapInfo: CGImageAlphaInfo.noneSkipFirst.rawValue) {
            ...
            let center = type == .single ? CGPoint.zero : CGPoint(x: imageWidth / 2, y: imageHeight / 2)
            ...
            context.fill(CGRect(x: 0.0, y: 0.0, width: 368, height: 216))
            ...
            if let cgImage = image.cgImage {
                context.draw(cgImage, in: CGRect(x: center.x, y: center.y, width: 368, height: 216))
            }
